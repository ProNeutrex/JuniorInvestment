    ...
    <div class="main-content">
        ...
        <div class="container-fluid">
            <div class="admin-latest-announcements">
                <div class="content">
                    <i icon-name="zap"></i><strong>Atenção, Administrador!</strong>
                </div>
                <p>Mantenha seu projeto atualizado e solicite recursos personalizados e exclusivos para impulsionar seu
                    sucesso no mercado.</p>
                <a href="https://designmp.net/suporte" target="_blank" class="cta-link">Saiba mais em Designmp.Net</a>
            </div>

            <div class="container pt-5 pb-5 text-center">
                <a href="/cron-job/investment" class="btn btn-primary btn-sm"><i class="fa-solid fa-plus"></i> Atualizar
                    manualmente os Investimentos</a>
                <hr>
                Este é um recurso para creditar manualmente todos os investidores. Utilize apenas se necessário ou se quer manter um controle manual sobre os rendimentos.
            </div>
            ...
            <div class="row">
                @canany(['deposit-action', 'withdraw-action', 'kyc-action'])
                    @if ($data['withdraw_count'] || $data['kyc_count'] || $data['deposit_count'])
                        <div class="col-xl-12">
                            <div class="admin-latest-announcements">
                                <div class="content"><i icon-name="zap"></i>{{ __("Explore what's important to review first") }}
                                </div>
                                <div class="content">
                                    @can('withdraw-action')
                                        @if ($data['withdraw_count'])
                                            <a href="{{ route('admin.withdraw.pending') }}" class="site-btn-xs red-btn"><i
                                                    icon-name="loader" class="spining-icon"></i>{{ __('Withdraw Requests') }}
                                                ({{ $data['withdraw_count'] }})
                                            </a>
                                        @endif
                                    @endcan
