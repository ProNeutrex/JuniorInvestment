    ...
    <div class="container position-relative z-5 pt-5 mt-4 mt-lg-5" data-bs-theme="dark">
        <div class="row justify-content-center text-center pt-md-2 pt-lg-0 pt-xl-2 pb-5 mb-md-2">
            <div class="col-lg-10 col-xl-9 col-xxl-8"><span
                    class="badge bg-light bg-opacity-10 fs-sm text-white px-3 mb-4">{{ $data['company_slogan'] ?? '' }}</span>
                <h1 class="display-3 mb-sm-4">{{ $data['hero_title'] }}</h1>
                <p class="text-body pb-3 mb-3 mb-lg-4">{{ $data['hero_content'] }}</p>
                <a class="btn btn-lg btn-warning"
                    href="/login">{{ $data['hero_button1_level'] }}</a>
            </div>
        </div>
        <div class="bg-primary rounded-4 p-2 mx-auto" style="max-width: 1160px;">
            <div id="image" class="overflow-hidden position-relative">
                <img id="image" src="{{ asset('frontend/theme_base/padraods/img/landing/product/video-cover.jpg') }}"
                     class="d-block rounded-5" alt="Video cover">
                <div class="position-absolute top-50 start-50 translate-middle z-2 p-4">
                    <button class="btn btn-icon btn-xl btn-primary rounded-circle" onclick="playVideo()"
            ...
            <div id="videoContainer" class="overflow-hidden position-relative d-none">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe id="videoFrame" class="embed-responsive-item" src="" height="550px" width="100%" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                </div>
            </div>
        </div>
        <script>
            function playVideo() {
                ...
                $('#videoFrame').attr('src', 'https://www.youtube-nocookie.com/embed/udMULpKcnn8?si=Wz2ddt1dRY38r87K&amp;controls=0&autoplay=1');
            }
        </script>
    </div>
</section>
