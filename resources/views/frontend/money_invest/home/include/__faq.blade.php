    ...
    <div class="container py-md-2 py-lg-3 py-xl-5 my-2 my-sm-3 my-md-4 my-xxl-5">
        <div class="row">
            <div class="col-md-4 text-center text-md-start">
                <h2 class="h1">{{ $data['title_small'] }}</h2>
                <p class="pb-3 pb-sm-4">{{ $data['title_big'] }}</p>
            ...
            <div class="col-md-8 col-lg-7 offset-lg-1">
                <div class="accordion" id="faq">
                    @foreach ($landingContent as $index => $content)
                        <div class="accordion-item bg-light">
                            <h3 class="accordion-header" id="heading{{ $index }}">
                                <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                        data-bs-target="#collapse{{ $index }}" aria-expanded="true"
                                        aria-controls="collapse{{ $index }}">{{ $content->title }}</button>
                            </h3>
                            <div class="accordion-collapse collapse" id="collapse{{ $index }}" aria-labelledby="heading{{ $index }}"
                                data-bs-parent="#faq">
                                <div class="accordion-body fs-sm">{{ $content->description }}</div>
                            </div>
                        </div>
                    @endforeach
                </div>
