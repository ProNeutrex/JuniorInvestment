...
if (! function_exists('getLocation')) {
    function getLocation()
    {
        ...
        $locationData = json_decode(curl_get_file_contents('http://ip-api.com/json/' . $ip), true);

        $currentCountry = collect(getCountries())->first(function ($value, $key) use ($locationData) {
            return isset($locationData['countryCode']) && $value['code'] == $locationData['countryCode'];
        });

        $location = [
            'country_code' => isset($currentCountry['code']) ? $currentCountry['code'] : null,
            'name' => isset($currentCountry['name']) ? $currentCountry['name'] : null,
            'dial_code' => isset($currentCountry['dial_code']) ? $currentCountry['dial_code'] : null,
