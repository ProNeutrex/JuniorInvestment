...
class CronJobController extends Controller
{
    ...
    {
        /* Comando do Cron /usr/bin/curl -s https://dominio/cron-job/investment 
        Atenção programador, você pode mover o Cron para o padrão Laravel se preferir. A opção do Controller é para melhor controle manual do cliente.
        */
        ...
        foreach ($ongoingInvestment as $invest) {
            ...
            if ($profitOffDays == null || !in_array($today, $profitOffDays)) {
                ...
                if ($invest->return_type == 'lifetime') {
                    ...
                    $tnxInfo = Txn::new($interest, 0, $interest, 'system', $schema->name . ' Rendimento', TxnType::Interest, TxnStatus::Success, null, null, $user->id);
                    ...
                    if ($invest->number_of_period > 0) {
                        if ($invest->number_of_period == 1) {
                            ...
                            if ($invest->capital_back == 1) {
                                ...
                                $tnxInfo = Txn::new($invest->invest_amount, 0, $invest->invest_amount, 'system', $schema->name . ' Retorno capital', TxnType::Refund, TxnStatus::Success, null, null, $user->id);
                                //notify newsletter
        ...
        /* Código de Cron pelo lado do servidor, ignorando datas e executando conforme o Cron 


        $ongoingInvestment = Invest::where('status', InvestStatus::Ongoing)->cursor();
        ...
        foreach ($ongoingInvestment as $invest) {
            ...
            if ($invest->return_type == 'lifetime') {
                ...
                $user->increment('profit_balance', $interest);
                $tnxInfo = Txn::new($interest, 0, $interest, 'system', $schema->name . ' Rendimentos', TxnType::Interest, TxnStatus::Success, null, null, $user->id);
                ...
                if ($invest->number_of_period > 0) {
                    ...
                    $user->increment('profit_balance', $interest);
                    $tnxInfo = Txn::new($interest, 0, $interest, 'system', $schema->name . ' Retorno investimento', TxnType::Interest, TxnStatus::Success, null, null, $user->id);
        ...
        */
        notify()->success('Cron Jobs executado com sucesso. Rendimentos creditados!', 'success');
        ...
        foreach ($referralRelationship as $relationship) {
            ...
            foreach ($filterReferrals as $referral) {
                ...
                if ($referral->type == 'deposit' && $referral->target_amount <= $totalDeposit && setting('deposit_referral_bounty', 'permission')) {
                    Txn::new($referralBonus, 0, $referralBonus, 'system', 'Bônus indicação ' . $targetName . ' Via ' . $user->full_name, TxnType::Referral, TxnStatus::Success, null, null, $provider->id, $user->id, 'User', [], 'none', $referral->referral_target_id, $referral->type);
                ...
                if ($referral->type == 'investment' && $referral->target_amount <= $totalInvest && setting('investment_referral_bounty', 'permission')) {
                    Txn::new($referralBonus, 0, $referralBonus, 'system', 'Bônus com ' . $targetName . ' Via ' . $user->full_name, TxnType::Referral, TxnStatus::Success, null, null, $provider->id, $user->id, 'User', [], 'none', $referral->referral_target_id, $referral->type);
                    $provider->increment('profit_balance', $referralBonus);
                }
            }
        }
        ...
        foreach (User::where('status', true)->get() as $user) {
            ...
            if ($eligibleRanks->isNotEmpty()) {
                ...
                foreach ($eligibleRanks as $rank) {
                    Txn::new($rank->bonus, 0, $rank->bonus, 'system', 'Bônus dê ' . $rank->ranking, TxnType::Bonus, TxnStatus::Success, null, null, $user->id);
