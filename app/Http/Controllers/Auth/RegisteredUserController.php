...
class RegisteredUserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        /* Bônus de ranking 1, reconfigurar
        if ($rank->bonus > 0) {
            Txn::new($rank->bonus, 0, $rank->bonus, 'system', 'Bônus de ranking '.$rank->ranking, TxnType::Bonus, TxnStatus::Success, null, null, $user->id);
            $user->increment('profit_balance', $rank->bonus);
        }
        */
        $shortcodes = [
            '[[full_name]]' => $input['first_name'].' '.$input['last_name'],
            '[[message]]' => '.Novo bônus de ranking.',
        ];
        ...
        if (setting('referral_signup_bonus', 'permission') && (float) setting('signup_bonus', 'fee') > 0) {
            ...
            Txn::new($signupBonus, 0, $signupBonus, 'system', 'Bônus de registro', TxnType::SignupBonus, TxnStatus::Success, null, null, $user->id);
