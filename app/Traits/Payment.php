...
use Payment\SuitPay\SuitPayTxn;
...
trait Payment
{
    ...
    protected function withdrawAutoGateway($gatewayCode, $txnInfo)
    {
        ...
        $notify = [
            'card-header' => 'Sacar dinheiro',
            'title' => $symbol.$txnInfo->amount.' saque solicitado com sucesso',
            'p' => 'Em breve seu pagamento será enviado',
            'strong' => 'ID Transação: '.$txnInfo->tnx,
            'action' => route('user.withdraw.view'),
            'a' => 'SOLICITAR NOVO PEDIDO',
            'view_name' => 'withdraw',
        ];
    ...
    protected function paymentNotify($tnx, $status)
    {
        ...
        $notify = [
            'card-header' => "$status depósito",
            'title' => "$symbol $tnxInfo->amount deposito $title",
            'p' => "O valor total deste item será adicionado em breve",
            'strong' => 'ID Depósito: '.$tnx,
            'action' => route('user.deposit.amount'),
            'a' => 'Depositar novamente',
        ];
